  ...
  checkAndHandleTalentLoss(student) {
    ...
    registerDefaultTalents(game, utils){
      ...
      this.registerTalent({
        ...
        this.registerTalent({
          name: '高考课内狂热者',
          description: "若选中题标签含 '高考课内'，在模拟赛中本题临时使 高考能力 翻倍（x2）。",
          color: '#00BCD4',
          prob: 0.08,
          beneficial: true,
          handler: function(student, eventName, ctx){
            try{
              ...
              const probObj = state.getProblem(pid);
              if(!probObj || !probObj.tags) return null;
              if(!probObj.tags.includes('高考课内')) return null;
              
              // 使用临时增益系统
              const dsBonus = student.knowledge_ds;
              student.applyTempModifier('knowledge_ds', student.getTempModifier('knowledge_ds') + dsBonus);
              
              return '高考课内狂热者发动：本题 高考课内能力 翻倍（本题临时）';
            }catch(e){ console.error('高考课内狂热者 天赋错误', e); }
            return null;
          }
        });
        ...
        this.registerTalent({
          name: '组合直觉',
          description: "在模拟赛选题时若题目含 '组合' 标签，有 30% 概率临时增加 组合 +60% 和 思维 +20%。",
          color: '#3F51B5',
          prob: 0.05,
          beneficial: true,
          handler: function(student, eventName, ctx){
            try{
              ...
              const probObj = state.getProblem(pid);
              if(!probObj || !probObj.tags) return null;
              if(!probObj.tags.includes('组合')) return null;
              if(getRandom() < 0.30){
                // 使用临时增益系统
                ...
                return '组合直觉发动：组合知识 +60%，组合 +20%（本题临时）';
              }
        ...
        this.registerTalent({
          name: '卡卡就过了',
          description: '比赛时已获得部分分时，一定概率直接通过此题',
        color: '#FFC107',
        prob: 0.03,
        kind: 'positive',
        beneficial: true,
        ...
        this.registerTalent({
          ...
          handler: function(student, eventName, ctx){
            try{
              ...
              return { action: 'reduce_difficulty', amount: 0.10, message: '键盘侠：书写难度-10%' };
        ...
        this.registerTalent({
          name: '代数魔法师',
          description: '对代数处理有特殊技巧，恒等变形能力和相关知识提升。',
          color: '#C2185B',
          prob: 0.05,
          kind: 'positive',
          beneficial: true,
          handler: function(student, eventName, ctx){
            try{
              ...
              if(!state) return null;
              const probObj = state.getProblem(pid);
              if(!probObj || !probObj.tags || !probObj.tags.includes('代数')) return null;
              
              if(getRandom() < 0.30){
                if(!student._talent_state['代数魔法师']){
                  const stringBonus = student._base_knowledge_string * 0.6; // +60%
                  const codingBonus = student._base_coding * 0.6;
                  student.applyTempModifier('knowledge_string', student.getTempModifier('knowledge_string') + stringBonus);
                  student.applyTempModifier('coding', student.getTempModifier('coding') + codingBonus);
                  student._talent_state['代数魔法师'] = true;
                  return '代数魔法师发动：代数知识与恒等变形+60%';
                }
              }
          ...
          handler: function(student, eventName, ctx){
            try{
              ...
              if(getRandom() < 0.20){
                ...
                if(topic === '高考课内') mainKnowledge = 'knowledge_ds';
                else if(topic === '几何') mainKnowledge = 'knowledge_graph';
                else if(topic === '代数') mainKnowledge = 'knowledge_string';
                else if(topic === '组合') mainKnowledge = 'knowledge_math';
                else if(topic === '数论') mainKnowledge = 'knowledge_dp';
          ...
          handler: function(student, eventName, ctx){
            try{
              ...
              if(getRandom() < 0.5){
                ...
                else if(topic === '图论') mainKnowledge = 'knowledge_dp';
