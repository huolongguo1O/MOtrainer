...
(function(global){
  ...
  function generateInternationalStudents(chineseStudents){
    ...
    const knowledgeAvg = { '高考课内':0,'几何':0,'代数':0,'组合':0,'数论':0 };
    let chinaCount = 0;
  ...
  const halfIndex = (game.week > halfBoundary) ? 1 : 0;
    ...
    // IOI 特殊标记（后续资格检查中使用）
    const isIOI = comp.name === 'IMO' && comp.nationalTeam;
    ...
    if(comp.name === 'IMO' && comp.nationalTeam){
      // 调试日志：显示 IOI 晋级资格信息
  ...
  function handleCompetitionResults(studentStates, config, originalComp, ineligibleStudents, internationalStudents){
    ...
    const results = [];
      for(let state of studentStates){
        ...
        let medal = null;
        if(originalComp && originalComp.name === 'CMO'){
  ...
  const halfIndex = (game.week > halfBoundary_local) ? 1 : 0;
    ...
    if(originalComp && originalComp.name === 'CMO' && halfIndex === 1){
    ...
    if(game.inNationalTeam && originalComp.name){
      ...
      // CTT 比赛：累计分数
      if(compName === '集训队一阶段测试1' || compName === '集训队一阶段测试2'){
    ...
    const careerResults = results.filter(r => {
      ...
      if(originalComp.name === 'IMO' && originalComp.nationalTeam && r.student.isInternational) return false;
      return true;
    });
  ...
  // 显示为两个维度的难度：思维难度 / 代码难度（使用该题最后一档的值代表题目总体难度）
    ...
    if(originalComp && originalComp.nationalTeam){
      if(config.name === '集训队一阶段测试1' || config.name === '集训队一阶段测试2' || config.name === '集训队二阶段'){
        html += `<p style="color:#666;">📊 本场成绩将计入总分，最终根据 CTST 总分决定IMO晋级</p>`;
      } else if(config.name === 'IMO'){
    ...
    participatedResults.forEach((r, index) => {
      ...
      for(let prob of r.problems){
        ...
        const isNOIOrAbove = compName === 'CMO' || compName === 'IMO';
        // 使用 originalComp 来检查 nationalTeam 属性
        const isNationalTeamSelection = originalComp && originalComp.nationalTeam && (compNameOriginal === '集训队一阶段测试1' || compNameOriginal === '集训队一阶段测试2' || compNameOriginal === '集训队二阶段');
  ...
  <td style="font-size:14px;font-weight:bold;">${(isFinite(Number(r.score)) ? Number(r.score).toFixed(1) : '0.0')}</td>
    ...
    if(config.name === 'CTS' && originalComp && originalComp.nationalTeam){
      ...
      if(game && game.nationalTeamResults){
        ...
        // 计算晋级线
        const maxPossible = 42 + 42 + 84; // CTT-day1-2 + CTT-day3-4 + CTS
        ...
        html += `<div style="font-size:16px;font-weight:bold;margin-bottom:8px;color:#1976d2;">📊 国家集训队总成绩（CTST）</div>`;
        html += `<table style="width:100%;font-size:13px;"><thead><tr><th>排名</th><th>姓名</th><th>二阶段总分</th><th>一阶段</th><th>总分</th><th>状态</th></tr></thead><tbody>`;
        
        for(let i = 0; i < totalScores.length; i++){
          let s = totalScores[i];
          const isQualified = qualified.includes(s.name);
          const rowStyle = isQualified ? 'background:#e8f5e9;font-weight:bold;' : '';
          const statusText = isQualified ? '<span style="color:#2e7d32;">✅ 预计晋级IMO</span>' : '<span style="color:#999;">未晋级</span>';
        ...
        window.closeModal = function(){
          ...
          if(game.inNationalTeam){
            ...
            if(game.pendingIOIQualificationCheck){
              ...
              try{
                console.log('【国家集训队】CTST 结束，开始计算IMO晋级');
          ...
          if(hasGoldMedal && config && config.name === 'CMO'){
            try{
              console.log('[国家集训队检测] 第二年CMO有金牌，设置待选择标志并显示国家集训队选择弹窗');
              // 设置标志，防止周更新时触发赛季结束
  ...
  function handleMockContestResults(studentStates, config, isPurchased, diffIdx){
    ...
    for(let state of studentStates){
      ...
      if(config.contestType === 'online'){
        ...
        if(diffIdx <= 0){
          gainRatio = CONTEST_GAIN_RATIOS['校内预选'] || gainRatio;
        } else if(diffIdx === 1){
          gainRatio = CONTEST_GAIN_RATIOS['预赛'] || gainRatio;
        } else if(diffIdx === 2){
          gainRatio = CONTEST_GAIN_RATIOS['联赛'] || gainRatio;
        } else {
          gainRatio = CONTEST_GAIN_RATIOS['CMO'] || gainRatio;
        }
      }
      ...
      for(let i = 0; i < state.problems.length; i++){
        ...
        for(let tag of tags){
          if(typeof s.addKnowledge === 'function'){
            ...
            // 兼容：直接修改字段
            if(tag === '高考课内') s.knowledge_ds = (s.knowledge_ds || 0) + perTagGain;
            if(tag === '几何') s.knowledge_graph = (s.knowledge_graph || 0) + perTagGain;
            if(tag === '代数') s.knowledge_string = (s.knowledge_string || 0) + perTagGain;
            if(tag === '组合') s.knowledge_math = (s.knowledge_math || 0) + perTagGain;
            if(tag === '数论' || tag === '数论') s.knowledge_dp = (s.knowledge_dp || 0) + perTagGain;
          }
        }
      }
      ...
      if(after.coding !== before.coding) deltas.push(`书写 ${after.coding - before.coding > 0 ? '+' : ''}${(after.coding - before.coding).toFixed(0)}`);
      ...
      const kDs = after.knowledge_ds - before.knowledge_ds; if(kDs) deltas.push(`高考课内 ${kDs>0?'+':''}${kDs}`);
      const kG = after.knowledge_graph - before.knowledge_graph; if(kG) deltas.push(`几何 ${kG>0?'+':''}${kG}`);
      const kS = after.knowledge_string - before.knowledge_string; if(kS) deltas.push(`代数 ${kS>0?'+':''}${kS}`);
      const kM = after.knowledge_math - before.knowledge_math; if(kM) deltas.push(`组合 ${kM>0?'+':''}${kM}`);
      const kD = after.knowledge_dp - before.knowledge_dp; if(kD) deltas.push(`数论 ${kD>0?'+':''}${kD}`);
