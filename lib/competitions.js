...
(function(global){
  ...
  class ContestSimulator {
      ...
      if(success){
        ...
        if(typeof s.triggerTalents === 'function'){
          ...
          try{
            ...
            if(tags.includes('高考课内')) knowledgeType = 'knowledge_ds';
            else if(tags.includes('几何')) knowledgeType = 'knowledge_graph';
            else if(tags.includes('代数')) knowledgeType = 'knowledge_string';
            else if(tags.includes('组合')) knowledgeType = 'knowledge_math';
            else if(tags.includes('数论')) knowledgeType = 'knowledge_dp';
    ...
    attemptSubtask(student, problem, subtask){
      ...
      try{
        ...
        if(problem && typeof problem.difficulty === 'number' && problem.difficulty > 2.0 * effectiveAbility){
          thinkingProb *= 0.45;
          codingProb *= 0.45;
          const msg = `${student.name} 在 T${(problem.id||0) + 1} 触发 难度压制：题目难度 ${problem.difficulty} > 2× 综合能力 ${effectiveAbility.toFixed(1)}，思维/书写通过概率被压制`;
    ...
    finish(){
      ...
      const mistakeApplicableContests = ['联赛', '预赛', 'CMO'];
    ...
    for(let i = 0; i < contestDef.numProblems; i++){
      ...
      try{
        if(contestDef && contestDef.contestType === 'online'){
          ...
          //if(otc !== '谜之竞赛' && otc !== '归零杯') forceSingle = true;
          forceSingle = true;
        }
      }catch(e){ /* ignore */ }
  ...
  function getOnlineContestDifficultyFactors(contestType, numProblems){
    ...
    if(contestType === "谜之CMO"){
      // 使用类似递增分布：0.4, 0.7, 1.0, 1.3, 1.6
      return [0.4, 0.7, 1.0, 1.3, 1.6, 2.0];
    }
    
    // Codeforces Div1: 5题，省选级难度(320)
    if(contestType === "谜之TST"){
      // 使用类似递增分布：0.4, 0.7, 1.0, 1.3, 1.6
      return [0.7, 1.0, 1.3, 1.6, 2.0, 3.0];
    }
    
    // 洛谷月赛: 4题，NOIP级难度(230)
    if(contestType === "NSMO"){
      // 使用类似 Div2 的分布，但4题：0.5, 0.8, 1.2, 1.5
      return [0.5, 0.8, 1.2, 1.5, 1.6, 1.7];
    }
