...
class Student {
  ...
  getKnowledgeByType(type){
    if(type==='高考课内') return this.knowledge_ds;
    if(type==='几何') return this.knowledge_graph;
    if(type==='代数') return this.knowledge_string;
    if(type==='组合') return this.knowledge_math;
    if(type==='数论' || type==='数论') return this.knowledge_dp;
    return 0;
  }
  addKnowledge(type,amount){
    ...
    if(type==='高考课内') this.knowledge_ds += safeAmount;
    else if(type==='几何') this.knowledge_graph += safeAmount;
    else if(type==='代数') this.knowledge_string += safeAmount;
    else if(type==='组合') this.knowledge_math += safeAmount;
    else if(type==='数论' || type==='数论') this.knowledge_dp += safeAmount;
  }
  // 统一接口：为思维/代码提供受控增幅方法
...
/* =========== 比赛数据复刻（两赛季） =========== */
const WEEKS_PER_HALF = Math.floor(SEASON_WEEKS / 2+10);
let competitions = [];
