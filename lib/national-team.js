...
(function(global){
  ...
  function showNationalTeamChoice(noiResults, noiMaxScore, passLine) {
    ...
    html += `<p>• 进入国家集训队后，将进行一阶段、二阶段比赛</p>`;
    html += `<p>• 没有金牌的学生将自动退队（不减少声誉）</p>`;
    html += `<p>• 根据一阶段+二阶段总分排名，入选国家队，参加IMO</p>`;
    html += `</div>`;
    html += `</div>`;
  ...
  function enterNationalTeam(goldStudents) {
    ...
    const nationalTeamComps = [
      {week: currentWeek + 2, name:"集训队一阶段测试1", difficulty:500, maxScore:600, numProblems:6, nationalTeam:true},
      {week: currentWeek + 3, name:"集训队一阶段测试2", difficulty:500, maxScore:600, numProblems:6, nationalTeam:true},
      {week: currentWeek + 4, name:"集训队二阶段", difficulty:520, maxScore:800, numProblems:8, nationalTeam:true}
  ...
  function calculateNationalTeamQualification() {
    ...
    html += `<div style="font-size:16px;font-weight:bold;margin-bottom:8px;">📊 总成绩（一阶段 + 二阶段）</div>`;
    html += `<table><thead><tr><th>排名</th><th>姓名</th><th>一阶段</th><th>二阶段</th><th>总分</th><th>结果</th></tr></thead><tbody>`;
    
    for(let i = 0; i < totalScores.length; i++){
      let s = totalScores[i];
      const isQualified = game.nationalTeamResults.ioiQualified.includes(s.name);
      const resultText = isQualified ? '✅ 晋级IMO' : '❌ 未晋级';
        ...
        confirmBtn.onclick = ()=>{
          ...
          if(qualified.length > 0){
            ...
            if(typeof global.pushEvent === 'function'){
              global.pushEvent({
                name: 'IMO晋级',
                description: `${qualified.length}名选手晋级IMO`,
                week: game.week
              });
            }
